...
const DayRow = (props: DayRow) => {
  ...
  return (
    <tr
      className={`[&:not(:last-child)]:border-b-2 border-gray-500 ${
        isToday ? "text-gray-50 font-bold opacity-100" : "opacity-60"
      }`}
    >
...
const Footer_Hours = (props: Hours) => {
  const { title, hours } = props;

  return (
    <>
      <div className="py-2 mb-2 uppercase font-light border-b-2">{title}</div>
